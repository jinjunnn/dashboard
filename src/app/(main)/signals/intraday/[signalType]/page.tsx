...
import { TrendingUp, Activity, BarChart3, Target, Shield, Square, Zap, RefreshCw } from "lucide-react";
...
interface SignalTypeContentProps {
  signalConfig: any;
  signals: any[];
  signalType: string;
}
...
// 信号类型页面组件 (现在接受预获取的数据)
function SignalTypeContent({ signalConfig, signals, signalType }: SignalTypeContentProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-muted-foreground text-xs">该类型的信号总数</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-muted-foreground text-xs">信号的平均置信度</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>该信号类型的详细配置和特征</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <div className="space-y-2">
              <div className="text-muted-foreground text-sm font-medium">信号类型</div>
              <div className="font-semibold">{signalConfig.displayName}</div>
            </div>
            <div className="space-y-2">
              <div className="text-muted-foreground text-sm font-medium">信号方向</div>
              <Badge variant="outline" className={getDirectionColor(signalConfig.direction)}>
                {getDirectionLabel(signalConfig.direction)}
              </Badge>
            </div>
            <div className="space-y-2">
              <div className="text-muted-foreground text-sm font-medium">风险等级</div>
              <Badge variant="secondary" className={riskLevelColors[signalConfig.riskLevel]}>
                {signalConfig.riskLevel}
              </Badge>
            </div>
            <div className="space-y-2">
              <div className="text-muted-foreground text-sm font-medium">信号分类</div>
              <Badge variant="outline">日内信号</Badge>
            </div>
            <div className="space-y-2">
              <div className="text-muted-foreground text-sm font-medium">主题颜色</div>
              <div className="flex items-center gap-2">
                <div className="h-4 w-4 rounded border" style={{ backgroundColor: signalConfig.color }} />
                <span className="font-mono text-sm">{signalConfig.color}</span>
              </div>
            </div>
            <div className="space-y-2">
              <div className="text-muted-foreground text-sm font-medium">系统名称</div>
...
// 导出主页面 (服务器组件)
export default async function IntradaySignalTypePage({ params }: IntradaySignalTypePageProps) {
  ...
  return (
    <Suspense fallback={<SignalTypeLoading />}>
      <SignalTypeContent signalConfig={data.signalConfig} signals={data.signals} signalType={signalType} />
    </Suspense>
  );
}
